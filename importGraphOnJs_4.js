...
async function loadAndVisualizeGraph() {
    ...
    if (!selectedTtlFile) {
        showNotification("Please select a TTL file first using the 'Select File' button.", 'warning');
        return;
    }

    const filePath = ttlFolder + selectedTtlFile + '.ttl';
    myChart.showLoading({ 
        text: `Loading and parsing: ${selectedTtlFile}.ttl...`,
        color: '#5470c6',
        textColor: '#333',
    ...
    try {
        ...
        parser.parse(ttlData, (error, quad, prefixes) => {
            ...
            if (quad) {
                ...
                myChart.hideLoading();
                if (parsingError) {
                    showNotification(`Error parsing TTL file '${selectedTtlFile}.ttl':\n${parsingError.message}\n\nCheck console for more details.`, 'error', 6000);
                    return; 
                }
                console.log(`TTL parsing complete for '${selectedTtlFile}.ttl'. Prefixes:`, prefixes);
                console.log("Total quads parsed:", store.size);
                if (store.size === 0) {
                    showNotification(`The TTL file '${selectedTtlFile}.ttl' is empty or does not contain any valid triples that could be visualized.`, 'warning', 6000);
                    ...
                    const fileInfoDisplay = document.getElementById('fileInfoDisplay');
                    if (fileInfoDisplay) fileInfoDisplay.textContent = `Type: ${selectedTtlFile}.ttl | Size: ${(response.headers.get('Content-Length') / 1024).toFixed(1)} KB | Graph Type: ${graphType}`;
                    return;
                }
                try {
                    const { nodes, links } = transformRdfToEcharts(store, prefixes);
                    console.log(`Transformed data for '${selectedTtlFile}.ttl'. Nodes: ${nodes.length}, Links: ${links.length}`);
                    let message = `Successfully loaded and visualized '${selectedTtlFile}.ttl'.`;
                    if (nodes.length === 0 && links.length === 0 && store.size > 0) {
                         ...
                         message = `Data from '${selectedTtlFile}.ttl' was parsed (${store.size} triples), but it resulted in an empty graph. Check if the data structure is suitable for the visualizer.`;
                    }
                    ...
                    showNotification(`Error processing graph data from '${selectedTtlFile}.ttl':\n${transformError.message}`, 'error', 6000);
                }
            }
        });
    } catch (e) {
        myChart.hideLoading();
        console.error("Error loading TTL file:", e);
        showNotification(`Error loading file '${selectedTtlFile}.ttl':\n${e.message}`, 'error', 6000);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // The fileInput logic is removed, so this block is no longer relevant.
    // The loadButton will now directly call loadAndVisualizeGraph.
    ...
    // The loadButton.disabled logic is removed as fileInput is gone.
    // The loadButton will be enabled only if a file is selected via the dropdown.

    // --- Initial file info ---
    // The initial file info display logic is removed as fileInput is gone.
    // The loadButton will now directly call loadAndVisualizeGraph.
